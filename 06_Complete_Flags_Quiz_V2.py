...
class Quiz:
    ...
    def next_question(self, flag_list, correct_ans, q_asked, num_correct):
        ...
        # Gets the Number correct and the number of Questions asked as well as the user's response
        response = self.response_entry.get()
        num_correct = self.q_correct.get()
        q_asked = self.asked_questions.get()
        correct_ans = self.valid_response.get()
...
class Quiz_Results:
    def __init__(self, partner, game_stats, game_history):
        ...
        self.export_button = Button(self.buttons_frame,
                                    ...
                                    command=lambda: self.export(game_stats, game_history))
    ...
    def export(self, game_stats, game_history):

        Export(self, game_stats, game_history)
...
class Export:
    def __init__(self, partner, game_stats, game_history):
        ...
        print("Quiz History", game_history)
        ...
        self.save_button = Button(self.save_cancel_frame, text="Save", width=5,
                                  command=partial(lambda: self.save_stats(partner, game_stats, game_history)))
    ...
    def save_stats(self, partner, game_stats, game_history):
